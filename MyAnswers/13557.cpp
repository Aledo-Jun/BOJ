//
// Created by june0 on 2024-04-23.
//
#include <bits/stdc++.h>
#include <bits/extc++.h>
...
public:
     ...
     * @param default_value The result of query that doesn't affect the other query result when performed <i>func</i> with
     */
    SegTree_iter(const vector<T> &v, T default_value = 0) : default_query(std::move(default_value)) {
        arr = v;
...
const ll INF = 1e15;
struct range_sum_max {
    ll all, lval, rval, sum_max;
    range_sum_max operator()(const range_sum_max& l, const range_sum_max& r) {
        return {l.all + r.all,
                max(l.lval, l.all + r.lval),
                max(r.rval, r.all + l.rval),
                max({l.sum_max, r.sum_max, l.rval + r.lval})};
    }
};
...
int32_t main() {
    ...
    SegTree_iter<ll, range_min> range_min_query(prefix, INF);
    SegTree_iter<ll, range_max> range_max_query(prefix, -INF);

    vector<range_sum_max> w(n + 1);
    for (int i = 0; i <= n; i++) w[i] = {v[i],v[i],v[i],v[i]};
    SegTree_iter<range_sum_max, range_sum_max> overlap_query(w, {0,-INF,-INF,-INF});
    ...
    while (m--) {
        ...
        ll ans;
        if (y1 < x2) { // no overlap
            ll l = range_min_query.query(x1 - 1, y1 - 1);
            ll r = range_max_query.query(x2, y2);
            ans = r - l;
        } else { // x1 <= x2 <= y1 <= y2
            ll l = range_min_query.query(x1 - 1, x2 - 1);
            ll r = range_max_query.query(x2, y2);
            ans = r - l;

            l = range_min_query.query(x1 - 1, y1 - 1);
            r = range_max_query.query(y1, y2);
            ans = max(ans, r - l);

            ans = max(ans, overlap_query.query(x2, y1).sum_max);
        }
        cout << ans << endl;
    }
