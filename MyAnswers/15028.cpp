//
// Created by june0 on 2024-10-14.
//
#include <bits/stdc++.h>
...
pair<Point,Point> diameterOfConvexHull(vector<Point> &v, Dist_func dist = dist_square<T,V>) {
...
int32_t main() {
    ...
    lld ans = 1e18;
    int k = 2;
    for (int i = 0, j = 1; i < m; i++, j = (j + 1) % m) {
        while (true) {
            auto curr_dist = distance_line(v[convex[i]], v[convex[j]], v[convex[k]]);
            auto next_dist = distance_line(v[convex[i]], v[convex[j]], v[convex[(k + 1) % m]]);
            if (next_dist > curr_dist) k = (k + 1) % m;
            else break;
        }
        ans = min(ans, distance_line(v[convex[i]], v[convex[j]], v[convex[k]]));
    }
