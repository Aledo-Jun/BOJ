...
public:
    min_bit(int sz) : tree(sz + 1, 1e9), n(sz) {}
...
int32_t main() {
    ...
    // i-th is excellent <=> there isn't 1 <= j < i s.t. v[j][1,2] < v[i][1,2]
    min_bit segTree(n + 1);

    int ans = 0;
    for (int i = 1; i <= n; i++) {
        if (segTree.query(v[i][1]) > v[i][2]) ans++;
        segTree.update(v[i][1], v[i][2]);
    }
