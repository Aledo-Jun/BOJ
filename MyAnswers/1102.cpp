//
// Created by june0 on 2024-09-18.
//
#include <bits/stdc++.h>
...
int32_t main() {
    ...
    int n;
    cin >> n;
    matrix<int> v(n, vector<int>(n));
    for (auto &row : v) for (auto &cell : row) cin >> cell;
    int state = 0;
    for (int i = 0; i < n; i++) {
        char x;
        cin >> x;
    ...
    auto solve = [&](auto &&self, int curr, int cnt) -> int {
        if (cnt >= p) return 0;
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < n; j++) {
                ...
                dp[curr] = min(dp[curr], self(self, curr | (1 << j), cnt + 1) + v[i][j]);
            }
        }
        return dp[curr];
    };

    int ans = solve(solve, state, __builtin_popcount(state));
    cout << (ans == 1e9 ? -1 : ans);
