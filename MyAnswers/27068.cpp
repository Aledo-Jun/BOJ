//
// Created by june0 on 2025-01-03.
//
#include <bits/stdc++.h>
...
int32_t main() {
    ...
    ll n, m, k, X;
    cin >> n >> m >> k >> X;
    matrix<ll> v(n, vector<ll>(m));
    for (auto &row : v)
        for (auto &e : row) cin >> e;

    const auto dxy = {pair{-1, 0}, pair{1, 0}, pair{0, -1}, pair{0, 1}};
    auto check = [&](ll val) -> bool { // O(nm)
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                if (visited[i][j] || abs(X - v[i][j]) <= val) continue;
                for (const auto &[dx, dy] : dxy) {
                    int ni = i + dx, nj = j + dy;
                    if (ni < 0 || ni >= n || nj < 0 || nj >= m) continue;
                    if (abs(v[i][j] - v[ni][nj]) > val) {
                        cnt += bfs(i, j);
                        break;
                    }
                }
