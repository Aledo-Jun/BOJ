//
// Created by june0 on 2024-05-06.
//
#include <bits/stdc++.h>
#include <bits/extc++.h>
...
private:
    ...
    T _query_r(int node, int s, int e, int l) {
        if (s == e) return tree[node].val;

        int m = (s + e) >> 1;
        if (l <= m) {
            return _query_r(tree[node].l, s, m, l) + tree[tree[node].r].val;
        } else {
            return _query_r(tree[node].r, m + 1, e, l);
        }
    }
...
public:
    ...
    explicit PersistentSegTree(int n = MAX + 5) : sz(n) {
        //tree.reserve(7'000'000); // n + q * log n
    ...
    T query(int i, int j, int l, int r) {
        T res1 = _query(roots[i], 0, sz - 1, l, r);
    ...
    T query_r(int i, int l) {
        return _query_r(roots[i], 0, sz - 1, l);
    }
};
...
int32_t main() {
    ...
    while (t--) {
        ...
        version.reserve(q.size());
        for (const auto& [x, y] : q) {
            version.emplace_back(x, PST.update(y, 1));
        }
        version.emplace_back(MAX + 100, PST.update(MAX, 1)); // padding for upperbound
        ...
        while (m--) {
            ...
            auto it1 = lower_bound(all(version), make_pair(x1, INT_MIN));
            auto it2 = upper_bound(all(version), make_pair(x2, INT_MAX));

            ans += PST.query(it1->second - 1, it2->second - 1, y1, y2);
        }
        cout << ans << endl;
        PST.clear();
...
// Use only one FenwickTree(BIT) with some clever way of performing offline queries
...
int32_t main() {
    ...
    while (tc--) {
        ...
        vector<tuple<int,int,int>> query[MAX + 10];
        for (int i = 0; i < m; i++) {
            int l, r, b, t;
            cin >> l >> r >> b >> t;
            query[l - 1].emplace_back(b, t, -1);
            query[r].emplace_back(b, t, 1);
        }

        FenWickTree rsq(MAX + 10);
        ll ans = 0;

        for (int x = 0; x <= MAX; x++) {
            for (const auto& y : q[x]) rsq.update(y, 1);
            for (const auto& [b, t, sgn]: query[x]) {
                ans += sgn * rsq.query(b, t);
            }
        }
        cout << ans << endl;
    }
