//
// Created by june0 on 2024-04-15.
//
#include <bits/stdc++.h>
#include <bits/extc++.h>
...
lld find_root(lld l, lld r, bool left) {
    int max_iterations = 100; // Maximum number of iterations

    while (max_iterations--) {
        lld m = (l + r) / 2;
        if ((f(m) > eps && left) || (f(m) < -eps && !left))
            r = m;
        else
            l = m;
    }
...
int32_t main() {
    ...
    lld low = -33, high = 33;
    lld l = find_root(low, high, true);
    lld r = find_root(low, high, false);

    //cout << (chrono::steady_clock::now() - start).count() / 1e9 << endl;
    // cout << l << ' ' << r << endl;
