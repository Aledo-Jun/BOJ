//
// Created by june0 on 2024-08-11.
//
#include <bits/stdc++.h>
...
int32_t main() {
    ...
    int n, k;
    cin >> n >> k;
    vector<int> v(n), diff(n + 1, 1e9), left(n + 1), right(n + 1);
    for (auto &e : v) cin >> e;
    for (int i = 1; i < n; i++) {
        diff[i] = v[i] - v[i-1];
        left[i] = i-1;
    ...
    priority_queue<pair<int,int>, vector<pair<int,int>>, greater<>> pq; // {val, l, r}
    for (int i = 1; i < n; i++) pq.emplace(diff[i], i);
    ...
    while (k--) {
        ...
        pq.emplace(diff[idx] = diff[left[idx]] + diff[right[idx]] - diff[idx], idx);
