...
std::vector<ll> prime_sieve(int n) {
    ...
    for (ll i = 2; i <= n; i++) {
        if (is_prime[i])
            sieve.push_back(i * i); // store square of prime
    }
    return sieve;
}

vector<ll> prime_square = prime_sieve(100'000);

int solve(int p_idx, ll divisor, ll m) {
    if (p_idx >= prime_square.size()) return 0;
    lll n_divisor = (lll)divisor * prime_square[p_idx];
    if (n_divisor > m) return 0;

    int res = 0;
    res += m / n_divisor;
    res += solve(p_idx + 1, divisor, m);
    res -= solve(p_idx + 1, n_divisor, m);
    return res;
}
