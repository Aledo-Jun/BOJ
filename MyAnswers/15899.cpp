//
// Created by june0 on 2024-06-01.
//
#include <bits/stdc++.h>
#include <bits/extc++.h>
...
public:
    ...
    int count_less_equal(int l, int r, int k) {
        int res = 0;
        for (l += size / 2, r += size / 2; l <= r; l >>= 1, r >>= 1) {
            if (l & 1) {
...
struct merge_sort_tree {
    vector<int> operator()(const vector<int>& l, const vector<int>& r) {
        ...
        std::merge(all(l), all(r), res.begin());
        return res;
    }
};
...
int32_t main() {
    ...
    vector<vector<int>> updates(c + 1);
    for (int i = 1; i <= n; i++) {
        updates[color[i]].emplace_back(S[i]);
    }
    vector<vector<int>> queries(c + 1);
    while (m--) {
        int v, k;
        cin >> v >> k;
        queries[k].emplace_back(v);
    }
    ...
    int ans = 0;
    for (int i = 1; i <= c; i++) {
        for (const auto& idx : updates[i]) fenWickTree.update(idx, 1);
        for (const auto& idx : queries[i]) {
            ans += fenWickTree.query(S[idx], T[idx]);
            if (ans >= MOD) ans -= MOD;
        }
    }
    cout << ans;
