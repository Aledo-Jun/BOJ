//
// Created by june0 on 2024-05-18.
//
#include <bits/stdc++.h>
#include <bits/extc++.h>
...
int32_t main() {
    ...
    vector<array<ll,3>> w(n); // counts B, B+C, B+2C
    for (int i = 0; i < n; i++) {
        if (i == 0) {
            w[i][0] = v[i];
        }
        else {
            ll stretchC = min(v[i], w[i - 1][0]);
            w[i][1] = stretchC;
            w[i - 1][0] -= stretchC;
            v[i] -= stretchC;

            ll stretch2C = min(v[i], w[i - 1][1]);
            w[i][2] = stretch2C;
            w[i - 1][1] -= stretch2C;
            v[i] -= stretch2C;
    ...
    ll ans = 0;
    for (const auto&[a, b, c]: w) ans += (a * B) + (b * (B+C)) + (c * (B+C+C));
    cout << ans;
