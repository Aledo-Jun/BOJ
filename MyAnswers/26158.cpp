//
// Created by june0 on 2025-01-21.
//
#include <bits/stdc++.h>
...
public:
     ...
     */
    SegTree_iter(const vector<T> &v, T&& default_query = {}) : SegTree_iter(v.size(), std::move(default_query)) {
        arr = v;
    ...
    int leftmost_greater_equal(int k) {
        for (int node = 1;;) {
            if (node >= n) return node - n;
            if (k <= tree[node << 1]) node = node << 1;
            else node = node << 1 | 1;
...
struct replacement {
    int operator()(int a, int b) const { return b; }
};
using range_sum_segtree = SegTree_iter<int, add, replacement>;
using range_max_segtree = SegTree_iter<int, mx, replacement>;
...
int32_t main() {
    ...
    vector<int> not_solved(n, 1);
    auto seg1 = range_sum_segtree(not_solved);
    auto seg2 = range_max_segtree(v);
    ...
    while (remain) {
        ...
        while (true) {
            ...
            int max_d = seg2.query(l, r);
            if (max_d <= d) break;

            int next = seg2.leftmost_greater_equal(d+1);
            assert(l <= next && next <= r);
            seg1.update(next, 0);
            seg2.update(next, 0);
            remain--;
