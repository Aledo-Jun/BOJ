//
// Created by june0 on 2024-07-06.
//
#include <bits/stdc++.h>
...
public:
    ...
    void calc_set() {
        grundyNumbers[1] = grundyNumbers[2] = grundyNumbers[3] = 1;
        for (int i = 4; i <= n; i++) {
            std::set<int> s;

            for (int j = 1; j <= i; j++)
                s.emplace(grundyNumbers[max(j - 3, 0)] ^ grundyNumbers[max(i - j - 2, 0)]);
