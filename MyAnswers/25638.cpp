//
// Created by june0 on 2024-06-22.
//
#include <bits/stdc++.h>
#include <bits/extc++.h>
...
int32_t main() {
    fastIO;
    int n, total_red{0}, total_blue{0};
    cin >> n;
    vector<bool> c(n + 1);
    for (int x, i = 1; i <= n; i++){
        cin >> x;
    ...
    function<pair<ll,ll>(int,int)> dfs = [&](int u, int p) -> pair<ll,ll> {
        ll red{0}, blue{0};
        for (const auto& [v, _]: g[u]) { // count at the subtree
            if (v == p) continue;
            auto [r, b] = dfs(v, u);
            ans[u] += red * b + blue * r;
            red += r;
        ...
        // outside the subtree
        if (c[u]) {
            ans[u] += red * (total_blue - blue) + blue * (total_red - red - 1);
            return {red + 1, blue};
        } else {
            ans[u] += red * (total_blue - blue - 1) + blue * (total_red - red);
            return {red, blue + 1};
        }
    };
    dfs(1, 0);
