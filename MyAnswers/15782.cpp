//
// Created by june0 on 2024-06-02.
//
#include <bits/stdc++.h>
#include <bits/extc++.h>
...
int32_t main() {
    ...
    vector<int> tree(n + 1), tmp(n + 1);
    for (int i = 1; i <= n; i++) cin >> tmp[i];
    for (int i = 1; i <= n; i++) tree[S[i]] = tmp[i];

    LazySegTree_iter<int,int,XOR,XOR,XOR_TS> segTree(tree, 0, 0);
    ...
    while (m--) {
        ...
        if (cmd == 1) {
            int x;
            cin >> x;
            cout << segTree.query(S[x], T[x]) << endl;
        } else {
            int x, y;
            cin >> x >> y;
            segTree.update(S[x], T[x], y);
        }
    }
