...
public:
    ...
    T query(int left, int right, int cnt){
        if (left == right) return left;

        int mid = (left + right) >> 1;
        T l = _query(1, 0, arr.size() - 1, left, mid);
        T r = _query(1, 0, arr.size() - 1, mid + 1, right);
        if (l >= cnt) return query(left, mid, cnt);
        return query(mid + 1, right, cnt - l);
    }
};
...
int32_t main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (cmd == 1){
            int rank;
            cin >> rank;
            int idx = segTree.query(0, MAX - 1, rank);
            cout << idx << '\n';
