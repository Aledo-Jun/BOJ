...
int32_t main() {
    ...
    cin >> k >> n;
    n--;
    vector<int> A(k), C(k + 1, 1);
    for (auto& e : A) cin >> e;
    for (int i = 1; i <= k; i++) cin >> C[i], C[i] *= -1;

    Utils::NTT::NTT<mint> ntt;

    auto P = ntt.convolution(A, C);
    P.resize(k);

    for (; n; n >>= 1) {
        auto Q = C;
        for (int i = 1; i <= k; i += 2) Q[i] *= -1;
        auto U = ntt.convolution(P, Q);
        auto V = ntt.convolution(C, Q);
        P.resize(k), U.resize(k * 2), V.resize(k * 2 + 1);
        for (int i = n & 1; i < k * 2; i += 2) P[i / 2] = U[i];
        C.resize(k + 1);
        for (int i = 0; i <= k; i++) C[i] = V[i * 2];
    }
    cout << P[0]/C[0];
