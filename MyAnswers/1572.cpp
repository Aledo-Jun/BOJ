 ...
 */
        ...
        public:
            ...
            // but it seems like using some non-recursive method.
            // I think there must be some way to do like this for other methods too.
            int query(int k) {
                int i = 1;
                while (i < (size >> 1)) {
                    if (tree[i * 2] >= k) i *= 2;
                    else k -= tree[i * 2], i = i * 2 + 1;
                }
                return i ^ (size >> 1);
            }
        }; // class SegTree
...
int32_t main() {
    ...
    src.emplace_back(0); // padding for convenience

    vector<int> v(1 << 16, 0);
    for (int i = 0; i < k; i++) v[src[i]]++;

    Utils::SegTree::SegTree<int> segTree(v);

    ll ans = 0;
    for (int i = 0; i < n - k + 1; i++){
        ans += segTree.query((k + 1) / 2);
        segTree.update(src[i], -1);
        segTree.update(src[i + k], +1);
    }
