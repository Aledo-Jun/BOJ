//
// Created by june0 on 2024-05-08.
//
#include <bits/stdc++.h>
#include <bits/extc++.h>
...
int32_t main() {
    ...
    bool flag = true;
    // IDK why, but using __int128(a.k.a. lll) has better performance.
    // But I think this idea is much better to know.
    for (int i = 0; i < n; i++) {
        ll g = gcd(v[i], ori[i]);
        ll a = v[i] / g;
        ll b = ori[i] / g;
        double sqrt_a = sqrt(a), sqrt_b = sqrt(b);
        if (sqrt_a != ll(sqrt_a) || sqrt_b != ll(sqrt_b)){
            flag = false;
            break;
        }
    }
