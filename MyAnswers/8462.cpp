...
private:
    std::vector<int> v;
    std::vector<query> q;
    ll res; // if the res can be maintained, use this.

    vector<int> cnt;
    void add(int i) {
        res += (2LL * cnt[v[i]] + 1) * v[i];
        cnt[v[i]]++;
    }
...
public:
    Mo() = default;
    Mo(const vector<int>& v, const vector<query>& q) : v(v), q(q) {
        sort(all(this->q));
        ans.resize(q.size());
        cnt.resize(MAX, 0);
    }
...
int32_t main() {
    ...
    for (const auto &e : solver.ans) cout << e << endl;
