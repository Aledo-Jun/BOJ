//
// Created by june0 on 2024-08-30.
//
#include <bits/stdc++.h>
...
int32_t main() {
    ...
    auto cost = [&](int i, int j) -> ll {
        return (ll)abs(b[j].first - a[i].first) * abs(b[j].second - a[i].second);
    };
    auto DnC = [&](auto &&self, int s, int e, int l, int r) -> ll {
        if (s > e) return 0;
        int i = (s + e) >> 1;
        ll opt, val = -1e18;
        for (int j = l; j <= r; j++) {
            ll c = cost(i, j);
            if (a[i].first > b[j].first || a[i].second > b[j].second) c = -c;
            if (c > val) {
                opt = j;
                val = c;
            }
        }
        return max({val, self(self, s, i-1, l, opt), self(self, i+1, e, opt, r)});
    };

    cout << DnC(DnC, 0, a.size()-1, 0, b.size()-1);
