//
// Created by june0 on 2024-06-05.
//
#include <bits/stdc++.h>
#include <bits/extc++.h>
...
array<vector<int>,3> EulerTour(const graph<int>& g, int root = 1) {
    int n = (int) g.size() - 1;
    vector<int> S(n + 1), T(n + 1), D(n + 1);
...
int32_t main() {
    ...
    for (int i = 1; i < n; i++) {
        ...
        if (s != "--") {
            if (s[0] == '-') queries.emplace_back(u, v);
            else queries.emplace_back(v, u);
        }
    }

    auto tour = EulerTour(g);
    auto s = tour[0], t = tour[1], d = tour[2];
    MarsSegTree<int> segTree(n + 1);

    auto query = [&](int u, int v, int val) {
        if (d[u] < d[v]) { // u is parent of v
            segTree.update(s[v], t[v], val);
        } else {
            segTree.update(1, s[u] - 1, val);
            segTree.update(t[u] + 1, n, val);
        }
    };
    ...
    while (q--) {
        ...
        string arrow;
        cin >> u >> arrow >> v;
        if (arrow == "<-") swap(u, v), arrow = "->";

        if (d[u] < d[v]) {
            if (dir[v] == 1) query(v, u, -1);
            if (dir[v] == -1) query(u, v, -1);
            dir[v] = arrow == "--" ? 0 : -1;
        } else {
            if (dir[u] == 1) query(u, v, -1);
            if (dir[u] == -1) query(v, u, -1);
            dir[u] = arrow == "--" ? 0 : 1;
        }

        if (arrow != "--") query(u, v, 1);

        cout << segTree.get_total_visible() - 1 << endl;
    }
