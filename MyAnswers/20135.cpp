...
template <typename flow_t, template<typename> class MaxFlow>
class MaxFlowLowerBound {
    MaxFlow<flow_t> maxFlow;
    vector<flow_t> demand, capacity, low;
...
public:
    ...
    void addEdge(int u, int v, flow_t lb, flow_t ub) {
        maxFlow.addEdge(u, v, ub - lb, capacity.size());
        demand[u] += lb;
        demand[v] -= lb;
        capacity.emplace_back(ub);
        low.emplace_back(lb);
    }
...
int32_t main() {
    ...
    int n, m;
    cin >> n >> m;
    vector<ll> demand(n + 1);
    for (int i = 1; i <= n; i++) cin >> demand[i], demand[i] = -demand[i];

    MaxFlowLowerBound<ll, Dinic_Large> solver(demand);

    for (int i = 0; i < m; i++) {
        int u, v, lb, ub;
        cin >> u >> v >> lb >> ub;
        solver.addEdge(u, v, lb, ub);
    }

    auto flow = solver.max_flow();
    if (flow == -1) return cout << -1 << endl, 0;
    else cout << 1 << endl;
