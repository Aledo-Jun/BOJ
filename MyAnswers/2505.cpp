//
// Created by june0 on 2024-09-28.
//
#include <bits/stdc++.h>
...
int32_t main() {
    ...
    if (check()) return cout << 1 << ' ' << 1 << endl << 1 << ' ' << 1, 0;
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (check() && !ans.empty()) {
            if (ans.size() == 1) cout << 1 << ' ' << 1 << endl;
            for (const auto &[a, b]: ans) cout << a << ' ' << b << endl;
            return 0;
        }
        if (cnt == 2) break;
    }
