...
constexpr ll DEFAULT_ASSIGN = -1e18;
struct Lazy {
    // first "assign", then "add"
    ll assign = DEFAULT_ASSIGN, add = 0;
...
struct TAG {
    bool operator()(const Node &node, const Lazy &lazy) const {
        const auto &[mn, mx, _] = node;
        const auto &[assign, add] = lazy;
        if (assign == DEFAULT_ASSIGN) return true;
        if (floor((lld)mn / assign) == floor((lld)mx / assign)) return true;
        if (mn + 1 == mx) return true;
        return false;
    }
};
struct BREAK {
    bool operator()(unused const Node &node, unused const Lazy &lazy) const {
        return false;
    }
};
struct MergeNode {
...
struct UpdateLazy {
    Lazy operator()(const Lazy &a, const Lazy &b, const Node &node) const {
        if (b.assign == DEFAULT_ASSIGN) {
            return b; // ADD query
        }
        if (floor((lld)node.mn / b.assign) == floor((lld)node.mx / b.assign)) {
            return { (ll)floor((lld)node.mn / b.assign), 0 };
        }
        if (node.mn + 1 == node.mx) {
            return { DEFAULT_ASSIGN, (ll)floor((lld)node.mn / b.assign) - node.mn };
        }
...
struct ApplyLazy {
    Node operator()(const Node &node, const Lazy &lazy, int len) const {
        ...
        if (assign == DEFAULT_ASSIGN) {
            return { node.mn + add, node.mx + add, node.sum + add * len };
        }
        return { assign + add, assign + add, (assign + add) * len };
    }
};
...
int32_t main() {
    ...
    while (m--) {
        ...
        if (op == 1) {
            int l, r, val;
            cin >> l >> r >> val;
            seg.update(l, r, { DEFAULT_ASSIGN, val });
        } else if (op == 2) {
            int l, r, val;
            cin >> l >> r >> val;
