...
array<vector<int>,3> EulerTour(const graph<int>& g, int root) {
    int n = (int) g.size() - 1;
    vector<int> S(n + 1), T(n + 1), D(n + 1, 0);
...
int32_t main() {
    ...
    auto [S, T, D] = EulerTour(g, src);
    FenWickTree<ll> segTree(n + 1);
    ...
    while (q--) {
        ...
        cin >> cmd >> x;
        if (cmd == 1) {
            segTree.update(S[x], 1);
        } else {
            cout << segTree.query(S[x], T[x]) * (D[x] + 1) << endl;
        }
    }
