//
// Created by june0 on 2024-08-06.
//
#include <bits/stdc++.h>
...
class MinCostMaxFlow {
    ...
    struct Edge {
        ...
        Edge(int u, int v, int cap, int rev, double cost) : u(u), v(v), cap(cap), rev(rev), cost(cost) {}
    };
...
private:
    ...
    bool bellmanFord(int s, int t) {
        ...
        while (!q.empty()) {
            ...
            is_in_q[u] = false;
            for (auto &e: g[u]) {
                if (e.cap > 0 && dist[e.v] > dist[u] + e.cost + 1e-9) {
...
public:
    ...
    pair<int, double> minCostMaxFlow(int s, int t) {
        int flow = 0;
        double cost = 0;
...
int32_t main() {
    ...
    for (int i = 0; i < n; i++) v.emplace_back(-v[i].first, v[i].second);
    // v[i + n] = y-axis symmetry of v[i]
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            mcmf.addEdge(i, j + n, 1, dist(v[i], v[j + n]) / 2.);
        }
    }

    const auto [flow, cost] = mcmf.minCostMaxFlow(s, t);
