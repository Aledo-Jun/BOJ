//
// Created by june0 on 2024-04-30.
//
#include <bits/stdc++.h>
#include <bits/extc++.h>
...
int32_t main() {
    ...
    for (int i = 1; i <= n; i++) {
        v[i] = lower_bound(all(compr), v[i]) - compr.begin();
    }
    ...
    for (const auto& [l, r, idx]: q) {
        while (s > l) add(--s, true);
        while (e < r) add(++e, false);
        while (s < l) del(s++, true);
        while (e > r) del(e--, false);
