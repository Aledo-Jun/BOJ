...
vector<int> getPartialMatch(const string_type &P) {
    ull m = P.size();
    vector<int> pi(m, 0);
    int match = 0;
    for (int i = 1; i < m; i++) {
...
template<class string_type>
vector<int> kmpSearch(const string_type &T, const string_type &P) {
    ull n = T.size();
    ull m = P.size();
    vector<int> result;
