//
// Created by june0 on 2024-04-19.
//
#include <bits/stdc++.h>
#include <bits/extc++.h>
...
void solve(int x, int y, int n) {
    if (n == 1) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 5 - 2 * i; j++) board[x + i][y + i + j] = '*';
        }
    }
    else if (n & 1) {
        int m = sz[n] - 1;
        ...
        for (int j = 0; j < 2 * m + 1; j++) board[x + m][y + j] = '*';

        solve(x + m / 2, y + m / 2 + 1, n - 1);
    }
    else {
        int m = sz[n] - 1;
        for (int j = 0; j < 2 * m + 1; j++) board[x][y + j] = '*';
        for (int i = 1; i <= m; i++) {
            board[x + i][y + i] = '*';
            board[x + i][y + 2 * m - i] = '*';
        }
        solve(x + 1, y + m / 2 + 1, n - 1);
    }
}
...
int32_t main() {
    ...
    cin >> n;
    sz[1] = 1;
    for (int i = 2; i <= n; i++) sz[i] = sz[i - 1] * 2 + 1;
    board.resize(sz[n], vector<char>(sz[n] * 2 - 1, ' '));
