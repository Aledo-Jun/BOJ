//
// Created by june0 on 2024-05-09.
//
#include <bits/stdc++.h>
#include <bits/extc++.h>
...
int32_t main() {
    ...
    k = n - k;
    deque<pair<int,int>> d; // increasing order
    int ans = INT_MAX;
    for (int i = 1; i < n; i++) {
        while (!d.empty() && d.back().first >= v[i] - v[i - 1]) d.pop_back();
        d.emplace_back(v[i] - v[i - 1], i);
        while (d.front().second <= i - k + 1) d.pop_front();
        if (i >= k - 1) ans = min(ans, d.front().first + v[i] - v[i - k + 1]);
    }
