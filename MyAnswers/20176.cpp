//
// Created by june0 on 2024-01-24.
// NOTE : In Most Cases, FFT is much faster than NTT
//
#include <bits/stdc++.h>
...
const int MAX_N = 60'000;

int32_t main() {
    fastIO;
    int na, nb, nc;
    vector<int> a(MAX_N + 1, 0), b, c(MAX_N + 1, 0);
    cin >> na;
    for (int i = 0; i < na; i++){
        int x; cin >> x;
        a[x + MAX_N / 2]++;
    ...
    auto conv = Utils::FFT::convolution(a, c);
