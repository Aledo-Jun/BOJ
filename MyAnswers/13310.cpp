//
// Created by june0 on 2024-10-11.
//
#include <bits/stdc++.h>
...
template<typename T = Point::value_type, typename V = wider_t<T>>
pair<Point,Point> diameterOfConvexHull(vector<Point> v) {
...
int32_t main() {
    ...
    for (auto &[pos, vel]: v) cin >> pos.x >> pos.y >> vel.x >> vel.y;

    auto check = [&](int time) {
        vector<Point> w; w.reserve(n);
        for (const auto &[pos, vel]: v) {
            w.emplace_back(pos + vel * time);
        }
        auto [p1, p2] = diameterOfConvexHull(std::move(w));
        return dist_square(p1, p2);
    };
