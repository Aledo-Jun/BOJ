//
// Created by june0 on 2024-06-28.
//
#include <bits/stdc++.h>
#include <bits/extc++.h>
...
int32_t main() {
    ...
    for (int m, i = 0; i < n; i++) {
        ...
        for (int j = 0; j < m; j++) {
            int x;
            cin >> x;
            v[i].algo.emplace_back(x);
        }
    }
    sort(all(v));

    int l = 0, r = 0;
    vector<int> algo(32, 0);
    int cnt = 1, OR, AND;
    ll ans = 0;

    for (const auto &e : v[r].algo) {
        algo[e]++;
    }
    while (r + 1 < n) {
        r++;
        cnt++;
        for (const auto &e : v[r].algo) algo[e]++;

        if (v[r].score - v[l].score > d) {
            for (const auto &e : v[r].algo) algo[e]--;
            r--;
            cnt--;
            OR = AND = 0;
            ...
            ans = max(ans, ll(OR - AND) * cnt);

            for (const auto &e : v[l].algo) algo[e]--;
            l++;
            cnt--;
        }
