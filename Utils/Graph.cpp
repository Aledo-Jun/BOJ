...
int minimumSpanningTree(const vector<Edge>& g){
    ...
    int res = 0;
    while (!minHeap.empty()) {
        auto e = minHeap.top(); minHeap.pop();
...
matrix<int> scc(graph<int>& g, graph<int>& g_r){
    ...
    while (!finishing_time.empty()){
        int u = finishing_time.top(); finishing_time.pop();
...
vector<int> preorderToPostorder(const vector<int>& preorder, int start, int end) {
    if (start > end) {
        return {};
    }

    if (start == end) {
        return {preorder[start]};  // Single node, return it as postorder
    }

    int rootValue = preorder[start];  // First element is the root
    vector<int> postorder;

    int i;
    for (i = start + 1; i <= end; i++) {
        if (preorder[i] > rootValue) {
            break;  // Found the partition point for the right subtree
        }
    }

    vector<int> leftSubtree = preorderToPostorder(preorder, start + 1, i - 1);
    vector<int> rightSubtree = preorderToPostorder(preorder, i, end);

    // Append left and right subtrees to the postorder traversal
...
vector<int> convertPreorderToPostorder(const vector<int>& preorder) {
    int n = preorder.size();
    return preorderToPostorder(preorder, 0, n - 1);
}
